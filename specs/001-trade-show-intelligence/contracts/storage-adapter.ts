...
/**
 * Data export structure for adapter migration
 */
export interface ExportedData {
  ...
  events: Event[]
  exportedAt: Date
  sourceAdapterType: 'LOCAL' | 'MYSQL' | 'HUBSPOT'
}
...
/**
 * Helper function to create storage adapter from configuration
 * @param config - Storage configuration
 * @param registry - Adapter implementation registry
...
export async function createStorageAdapter(
  ...
  if (!factory) {
    throw new Error(`Unknown storage adapter type: ${config.adapterType}`)
  }
